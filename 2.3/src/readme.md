...
Столкнулся с критическим падением производительности из-за некорректной работы с генератором случайных чисел. Изначальная реализация инициализировала seed внутри функции перестановки: ```seed = time(NULL) ^ pthread_self()```
